class Frontend::InformationController < FrontendController
  ...
  def comment
    ...
    if comment.save
      redirect_to frontend_information_path(@information)
    end
  end

  def index
    @information_types = InformationType.select("id, name, parent_id, top_number").where(parent_id: nil).order("created_at desc")

    @information_hotest_images = Information.select('id, name, image_url').unscope(:where).where(types: 7).order("created_at desc").limit(2)

    @precise_queries = PreciseQuery.all

    @infors = Information.where(is_top: true).where.not(image_url: nil).order("str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc").limit(4)
  ...
  def more_information
    name = params[:name] || '头条' 
    it = InformationType.where(name: name).first

    per = (it.name == '头条' || it.name == '天天护理') ? 12 : 8
    page = params[:page] || 2

    informations = Information.unscope(:where).where(types: 0, is_top: false, information_type_id: it.id).order("created_at desc").page(page).per(per)
  ...
  def information_list
    @information = DiseaseInfo.where(disease_info_type_id: params[:disease_info_type_id]).all
  end

  def show
    @guess_infos = Information.guessed.order("str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc").limit(12)

    @recommended_infos = Information.unscope(:where).select('id, name').where.not(id: params[:id]).order("str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc").limit(8)

    @hot_images = Information.unscope(:where).select('id, name, image_url').where.not(id: params[:id], image_url: nil).order("str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc").limit(8)
